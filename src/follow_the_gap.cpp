...
public:
    follow_the_gap() :
            ...
            lidar_sub_(node_handle_.subscribe("opp_id/scan", 100, &follow_the_gap::scan_callback, this)),
            drive_pub_(node_handle_.advertise<ackermann_msgs::AckermannDriveStamped>("opp_id/drive", 100)),
            truncated_(false), truncated_coverage_angle_(3.14), max_accepted_distance_(3.0),
            smoothing_filter_size_(3), bubble_radius_(0.1), steering_angle_reactivity_(0.6),
            error_based_velocities_({{"low", 5.0}, {"medium", 4.0}, {"high", 1.5}})
    {
                                               ...
                                               const double closest_value)
    {
        ...
        /*
        auto temp = (best_point_steering_angle*steering_angle_reactivity_)/static_cast<double>(closest_value);
        if(temp < -1.57) temp = -1.57;
        if(temp > 1.57) temp = 1.57;
        const auto distance_compensated_steering_angle = temp;
        */
